...
# Fit lại LabelEncoder cho metric để có Jaccard
le_upgrade = dict(le)
le_upgrade["m"] = LabelEncoder().fit(list(METR_dict.keys()))

# Compute the intra distances
intra_df = IntraDistance(METR_dict, le_upgrade, 1, progress_bar=True).fit(df_h)
...
inter_df = InterDistance(
    METR_dict, le_upgrade, set_class=0, n_samples=n_samples, progress_bar=True
).fit(df_h)
df_d = pd.concat([intra_df, inter_df])

print(f"Number of total inter distances = {len(inter_df)}")

# Lưu le_upgrade để demo03 và demo04 dùng
dump(value=le_upgrade, filename="./demo_outputs/LabelEncoders_upgrade.bz2", compress=9)

# Combine distances and save to disk
dump(value=df_d, filename="./demo_outputs/Distances_upgrade.df.bz2", compress=9)
